//Find 4x4 Magic Squares with 1 in the upper left using BackTracking.
...
int do_row1(int v[])
{
	...
	for(int k = j + 1; k <= 15; k++)
		if(v[i] + v[j] + v[k] == 33) //Check if the corresponding triple of values sums correctly.
		{
			...
			while(next_permutation(v + 1, v + 4));						//Scramblings of v[1..3]
...
int do_col2(int v[])	//Columns 2 already has v[1] & v[4]
{
	int count = 0;
	int s = 33 - v[3] - v[6];
	if(s > 16 || s < 2) return 0;			//Too high already or too low
...
int do_col1(int v[])
{
	...
	if(v[12] + v[14] + v[7] == 33 || v[12] + v[15] + v[7] == 33)
		return 1;
	return 0;
}
